...
func init() {
	android.RegisterModuleType("scoop_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = scoopExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = scoopExpandVariables(ctx, dep_file)
	...
	cmd := scoopExpandVariables(ctx, String(g.properties.Cmd))
